...
describe 'Users API' do
  ...
  it 'returns a single users events with attributes' do
    ...
    expect(user_events.lenght).to eq(2)
    expect(user_events.first[:id]).to eq(@event_1.id.to_s)
    expect(user_events.first[:name]).to eq(@event_1.name)
    expect(user_events.first[:city]).to eq(@event_1.city)
    expect(user_events.first[:state]).to eq(@event_1.state)
    expect(user_events.first[:price]).to eq(@event_1.price)
    expect(user_events.first[:event_type]).to eq(@event_1.event_type)
    expect(user_events.first[:start_date]).to eq(@event_1.start_date)
    expect(user_events.first[:end_date]).to eq(@event_1.end_date)
    expect(user_events.first[:description]).to eq(@event_1.description)
    expect(user_events.first[:image_url]).to eq(@event_1.image_url)
    expect(user_events.first[:event_url]).to eq(@event_1.event_url)
    expect(user_events.first[:video_url]).to eq(@event_1.video_url)
    expect(user_events.last[:id]).to eq(@event_2.id.to_s)
    expect(user_events.last[:name]).to eq(@event_2.name)
  end
end
